"""Module for pyvelora/tests/tensor_test.py."""
...
def test_tensor_helper_methods_delegate_through_linalg_ops():
    t1 = Tensor(np.arange(8).reshape(2, 2, 2))
    t2 = Tensor(np.arange(8, 16).reshape(2, 2, 2))

    assert np.array_equal(t1.outer(t2).data, np.multiply.outer(t1.data, t2.data))
    assert np.array_equal(t1.inner(t2).data, np.inner(t1.data, t2.data))
    assert np.array_equal(t1.tensordot(t2, axes=1).data, np.tensordot(t1.data, t2.data, axes=1))
    assert np.array_equal(t1.kron(t2).data, np.kron(t1.data, t2.data))
