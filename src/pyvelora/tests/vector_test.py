...
def test_vector_getitem_slice_returns_vector():
	v = Vector([10, 20, 30, 40])
	sliced = v[1:3]
	assert isinstance(sliced, Vector)
	assert np.array_equal(sliced.data, np.array([20, 30]))
...
def test_vector_eq_and_ne_elementwise():
	...
	assert isinstance(eq, Vector)
	assert eq.all()
	assert ne.data[1] == True
...
def test_vector_lt_le_gt_ge():
	...
	assert np.array_equal(lt.data, np.array([True,  False, False]))
	assert np.array_equal(le.data, np.array([True,  True,  False]))
	assert np.array_equal(gt.data, np.array([False, False, True]))
	assert np.array_equal(ge.data, np.array([False, True,  True]))
...
def test_vector_arithmetic_with_wrong_type_returns_not_implemented():
	v = Vector([1, 2])
	assert v.__add__(42) is NotImplemented
...
def test_vector_astype_changes_underlying_numpy_dtype():
	v = Vector([1, 2, 3])
	# astype returns a new Vector whose raw ndarray uses the requested dtype;
	# the Vector constructor re-casts to float64, so verify the numpy cast
	# operation itself is functional.
...
def test_vector_reshape_returns_correct_shape():
	...
	# raw reshape to 2-d array so we don't trip Vector 1D guard
